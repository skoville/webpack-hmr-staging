...
export class NodeCompilerManagerRegistryModule extends AbstractCompilerManagerModule {
    ...
    // allow us to put the bundleId in GET request headers, which could be passed her, we must ask that clients of this library ensure that no
    ...
    protected async readFile(request: ReadFileRequest): Promise<AbstractFileStream> {
        ...
        if (readStreams.length === 0) {
            ...
            // In the future once we are able to send the bundleId as part of the GET request header, then it won't matter
